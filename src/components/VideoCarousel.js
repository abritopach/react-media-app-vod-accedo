...
const VideoCarousel = ({ videos, onHandleSelectVideo, selectedVideo, onHandleKeyPress }) => (
    <div className="col-md-12">
        ...
        <div id="video-thumbnail" className="video-thumbnail">
            {videos.map((video, i) => (
                <div key={i} onClick={onHandleSelectVideo.bind(this, video)} onKeyDown={onHandleKeyPress.bind(this, video, i)}>
                    <video tabIndex="0" id={video.id} poster={video.thumbnail} src={video.mediaUrl} alt={video.description}/>
                    <p>{video.title}</p>
                </div>
            ))}
        </div>
